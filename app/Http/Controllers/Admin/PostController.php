...
class PostController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('featured_image')) {
            try {
                ...
                \Log::info('Gambar berhasil diupload: ' . $data['featured_image']);
                \Log::info('URL gambar: ' . ImageService::url($data['featured_image']));
            } catch (\Exception $e) {
                \Log::error('Gagal mengupload gambar: ' . $e->getMessage());
        ...
        if ($request->hasFile('featured_image')) {
            try {
                ...
                if ($post->featured_image) {
                    ...
                    \Log::info('Gambar lama dihapus: ' . $post->featured_image);
                }
                
                // Upload new image
                $data['featured_image'] = $this->uploadImage($request->file('featured_image'));
                \Log::info('Gambar baru diupload: ' . $data['featured_image']);
                \Log::info('URL gambar baru: ' . ImageService::url($data['featured_image']));
                
            } catch (\Exception $e) {
                \Log::error('Gagal mengupdate gambar: ' . $e->getMessage());
                return redirect()->back()
                    ->withInput()
    ...
    public function destroy(Post $post)
    {
        try {
            ...
            if ($post->featured_image) {
                if (ImageService::delete($post->featured_image)) {
                    \Log::info('Gambar berhasil dihapus: ' . $post->featured_image);
                } else {
                    \Log::warning('Gagal menghapus gambar: ' . $post->featured_image);
                }
            }

            $post->delete();
            \Log::info('Post berhasil dihapus: ' . $post->id);
            ...
            return redirect()->route('admin.posts.index')
                ->with('error', 'Terjadi kesalahan saat menghapus post: ' . $e->getMessage());
        }
    }
    ...
    {
        return ImageService::upload($file, 'posts');
    }
